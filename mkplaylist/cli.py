...
@cli.command()
@click.option('--spotify-only', is_flag=True, help='Only sync data from Spotify')
@click.option('--lastfm-only', is_flag=True, help='Only sync data from Last.fm')
@click.option('--full', is_flag=True, help='Perform a full sync instead of incremental')
@click.option('--days', type=int, default=config.DEFAULT_SYNC_DAYS, 
...
@click.option('--collaborative', is_flag=True, help='Make the playlist collaborative')
@click.option('--replace', is_flag=True, help='Replace all tracks in the playlist (default: append)')
@click.option('--limit', type=int, help='Maximum number of tracks to include')
def create(playlist_name: str, criteria: str, description: Optional[str], 
           public: bool, collaborative: bool, replace: bool, limit: Optional[int]):
    """
...
@click.option('--sort', type=click.Choice(['name', 'date', 'updated']), default='updated',
...
def list(format: str, sort: str):
    ...
    click.echo("No playlists found. Use 'mkplaylist create' to create a playlist.")
