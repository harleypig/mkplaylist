...
from mkplaylist.database.models import Base, Track, Playlist, PlaylistTrack, ListeningHistory
...
class DatabaseManager:
    ...
    def add_track(self, track_data: Dict[str, Any]) -> Track:
        ...
        try:
            ...
            if spotify_id:
                ...
                track = Track(**track_data)
                session.add(track)
                logger.debug(f"Added new track without Spotify ID: {track.name} by {track.artist}")
    ...
    def add_track_to_playlist(self, playlist_id: int, track_id: int, position: Optional[int] = None,
                             added_at: Optional[datetime] = None) -> PlaylistTrack:
        """
        ...
        try:
            ...
            playlist_track = session.query(PlaylistTrack).filter_by(
                playlist_id=playlist_id, track_id=track_id).first()
            ...
            if playlist_track:
                ...
                logger.debug(f"Updated track position in playlist: {playlist_id}, track: {track_id}")
            else:
                ...
                logger.debug(f"Added track to playlist: {playlist_id}, track: {track_id}")
    ...
    def add_listening_event(self, track_id: int, played_at: datetime, source: str = "lastfm") -> ListeningHistory:
        """
        ...
        try:
            ...
            session.commit()
            logger.debug(f"Added listening event for track: {track_id}, played at: {played_at}")
            return history
        except SQLAlchemyError as e:
            session.rollback()
