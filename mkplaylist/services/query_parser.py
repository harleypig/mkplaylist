...
class QueryParser:
    ...
    def __init__(self):
        ...
        self.patterns = {
            'recently_added': re.compile(r'(\d+)\s+(?:most\s+)?recently\s+added\s+songs'),
            'last_played': re.compile(r'(\d+)\s+(?:most\s+)?(?:recently\s+played|last\s+played)\s+songs'),
            ...
            'played_days': re.compile(r'songs\s+played\s+in\s+the\s+last\s+(\d+)\s+days'),
        }
